...
function convertNode(xmlNode) {
  const id2 = xmlNode.getAttribute("ID") || `genid-${Math.random().toString(36).substr(2, 9)}`;
  ...
  const nodeData = {
    id: id2,
    name: text,
    // d3 often uses 'name' for display text
    attributes: {},
    children: []
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function executeSearch(searchTerm) {
    ...
    if (searchTerm.trim() === "") {
      renderGraph(currentMindMapData, "");
    } else {
  ...
  const debouncedExecuteSearch = debounce(executeSearch, 1e3);
  ...
  function filterMindMapData(node, searchTerm) {
    ...
    if (nodeMatches || descendantMatches) {
      ...
      if (descendantMatches) {
        ...
        newNode.children = filteredChildren.length > 0 ? filteredChildren : void 0;
      }
  ...
  function renderGraph(data, searchTerm = "") {
    ...
    const height = graphContainerElement.clientHeight || 400;
    const svg = select_default2(graphContainerElement).append("svg").attr("width", width).attr("height", height).call(zoom_default2().on("zoom", (event) => {
    ...
    const treeLayout = tree_default().nodeSize([nodeHeight, nodeWidth]);
    treeLayout(root2);
    g.selectAll(".link").data(root2.links()).enter().append("path").attr("class", "link").attr("d", linkHorizontal().x((d) => d.y).y((d) => d.x)).style("fill", "none").style("stroke", "#ccc").style("stroke-width", "1.5px");
    const node = g.selectAll(".node").data(root2.descendants()).enter().append("g").attr("class", (d) => `node ${d.children ? "node--internal" : "node--leaf"}`).attr("transform", (d) => `translate(${d.y},${d.x})`);
    ...
    node.append("text").attr("dy", ".35em").attr("x", (d) => d.children ? -13 : 13).style("text-anchor", (d) => d.children ? "end" : "start").text((d) => d.data.name).style("font-size", "10px").call(wrapText, textMaxWidth, lineHeight);
    const firstNode = root2.descendants()[0];
    if (firstNode) {
      ...
      const graphHeight = maxX - minX + nodeHeight;
      const graphWidth = maxY - minY + nodeWidth;
      let initialScale = Math.min((height - 40) / graphHeight, (width - 40) / graphWidth, 1);
      if (initialScale > 1) initialScale = 1;
      const translateX = (width - graphWidth * initialScale) / 2 - minY * initialScale + 20;
      const translateY = (height - graphHeight * initialScale) / 2 - minX * initialScale + 20;
      const initialTransform = identity2.translate(translateX, translateY).scale(initialScale);
      select_default2(graphContainerElement).select("svg").call(zoom_default2().transform, initialTransform);
    }
  }
  function wrapText(textSelection, maxWidth, lineHeight) {
    textSelection.each(function(d) {
      ...
      let line = [];
      let lineNumber = 0;
      const x2 = text.attr("x");
      const initialDy = parseFloat(text.attr("dy")) || 0;
      text.text(null);
      let tspan = text.append("tspan").attr("x", x2).attr("dy", initialDy + "em");
      while (word = words.pop()) {
        ...
        if (tspan.node().getComputedTextLength() > maxWidth && line.length > 1) {
          ...
          line = [word];
          lineNumber++;
          tspan = text.append("tspan").attr("x", x2).attr("dy", lineNumber === 0 && initialDy !== 0 ? initialDy + "em" : lineHeight).text(word);
        }
      }
    });
  }
